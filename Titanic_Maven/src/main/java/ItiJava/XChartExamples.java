...
public class XChartExamples {
	public static void main(String[] args) {
		...
		xChartExamples.pieChartPassengerGender(allPassengers);
		xChartExamples.pieChartPassengerSurvived(allPassengers);
		xChartExamples.pieChartPassengerEmbarked(allPassengers);
	...
	public void pieChartPassengerGender(List<TitanicPassenger> passengerList) {
	...
	public void pieChartPassengerSurvived(List<TitanicPassenger> passengerList) {
		Map<String, Long> result = passengerList.stream().collect(
				Collectors.groupingBy(
	...
	public void pieChartPassengerEmbarked(List<TitanicPassenger> passengerList) {
		Map<String, Long> result = passengerList.stream().collect(
				Collectors.groupingBy(
						TitanicPassenger::getEmbarked, Collectors.counting()
				)
		);
		PieChart chart = new PieChartBuilder().width(800).height(600).title(getClass().getSimpleName()).build();
		Color[] sliceColors = new Color[]{new Color(250, 250, 250), new Color(0, 0, 250)};
		chart.getStyler().setSeriesColors(sliceColors);
		chart.addSeries("Embarked S", result.get("S"));
		chart.addSeries("Ebmarked C", result.get("C"));
		new SwingWrapper(chart).displayChart();
	}
